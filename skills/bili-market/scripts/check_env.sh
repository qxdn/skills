...
# -*- coding: utf-8 -*-
#
# bili-market 前置环境检查脚本
#
# 用途：运行 bili_market.py 之前，先执行本脚本确认环境就绪。
#
# python 环境选择（按优先级）：
...
#
# httpx 缺失时会自动安装到上面选定的环境中。
#
# 注意：Linux/Docker 环境需要 python 自带 venv 支持（Debian/Ubuntu 为 python3-venv 包，
# 未安装时本脚本会给出对应的安装命令提示）。
#
# 使用方式：
#   bash skills/bili-market/scripts/check_env.sh
#
# 退出码：0 = 环境就绪；1 = 环境异常（需人工处理）
# 成功时最后一行会输出运行 bili_market.py 应使用的 python 绝对路径。
...
# 检查指定 python 是否能导入 httpx；不能则自动安装并二次验证
# 用法: ensure_httpx <python路径>
ensure_httpx() {
    local py="$1"
    if "$py" -c "import httpx" >/dev/null 2>&1; then
        echo "[OK] httpx $("$py" -c "import httpx; print(httpx.__version__)")"
        return 0
    fi
    echo "[安装] 未检测到 httpx，正在安装到当前环境..."
    if "$py" -m pip install httpx && "$py" -c "import httpx" >/dev/null 2>&1; then
        echo "[OK] httpx $("$py" -c "import httpx; print(httpx.__version__)") 安装成功"
        return 0
    fi
    echo "[错误] httpx 安装失败，请手动执行: \"$py\" -m pip install httpx" >&2
    return 1
}
...
# ---------- 2. 检查 httpx ----------
ensure_httpx "$PY" || exit 1

# ---------- 3. 输出运行 bili_market.py 应使用的 python 路径 ----------
echo "[OK] 环境检查通过"
